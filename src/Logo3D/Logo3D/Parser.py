...
def parse_to_commands(tokens, index, open_bracket_count, debug_mode):
    commands = []
    while (index < len(tokens)):
        ...
        if (current_token == PENUP_STR):
            commands.append((PENUP))
        elif (current_token == PENDOWN_STR):
            commands.append((PENDOWN))
        elif (current_token == HOME_STR):
            commands.append((HOME))
        elif (current_token in [FD_STR, BK_STR, RT_STR, LT_STR, UT_STR, DT_STR]):
            index += 1
            if (index == len(tokens)):
            ...
            try:
                ...
                (repeat_commands, updated_index, new_open_bracket_count) = parse_to_commands(tokens, index, open_bracket_count, debug_mode)
